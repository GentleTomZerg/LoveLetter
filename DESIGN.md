...
- C→S: `createRoom {name, capacity}` · `joinRoom {roomCode, name}` · `playCard {which: 0|1}` · `choice {…}` · `nextRound` · `rematch` · `resume {playerId, lastEventId}` · `chat {text}` · `roomList` (Q22 — the room directory request)
- S→C: `hello {playerId, roomCode}` · `snapshot {view, lastEventId}` · `event {id, event}` (stream) · `chat {from, name, text}` · `chatLog {messages}` (on resume) · `roomList {rooms}` (on request, and pushed to browsing sockets on the lobby-relevant transitions — Q22) · `error`
