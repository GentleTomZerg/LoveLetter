...
function seatEl(id: string): HTMLElement | null {
  // Ticket 35: the viewer's seat lives in the dock (`.dock-seat`), not the
  // ring — both carry `.seat` + `data-player-id`, so a global query finds
  // whichever one belongs to the id.
  return document.querySelector(`.seat[data-player-id="${CSS.escape(id)}"]`);
}
...
function destRect(id: string, pile: boolean): { x: number; y: number } | null {
  const el = pile ? document.querySelector(`.seat[data-player-id="${CSS.escape(id)}"] .pile img`) ?? seatEl(id) : seatEl(id);
